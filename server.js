...
// =========================
// ENDPOINT: Buscar productos activos y en stock
// =========================
app.get("/products", async (req, res) => {
  try {
    ...
    // GraphQL — buscamos productos activos (sin published_status)
    const graphqlQuery = {
      query: `
        {
    ...
    // 🔍 Filtrado inteligente: coincidencias parciales (título o vendor)
    const filtered = allProducts.filter((p) => {
      const text = `${p.title} ${p.vendor}`.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      return text.includes(normalized.toLowerCase());
    });

    // 🧠 Fuzzy backup si no hay resultados
    const fallback = allProducts.filter((p) => {
      const title = p.title.toLowerCase();
      return (
        normalized.length > 2 &&
        title.startsWith(normalized.slice(0, 3))
      );
    });

    const results = filtered.length > 0 ? filtered : fallback;

    // 🧩 Mapeo final solo con productos disponibles
